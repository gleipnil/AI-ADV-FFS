...
### Phase 3 - 🚧 進行中
- [x] 記憶のカケラ基礎実装（授与・永続化・能力復元）
- [x] 真実ルート定義（3ルート）
- [x] Repository パターン実装（将来のSQL移行対応）
- [x] 20ターン制限の削除（シーンベース進行に一本化）
...
## 🆕 Phase 3 最新実装（2026-01-06）

### 記憶のカケラシステム

**Repository パターン**でデータアクセスを抽象化し、将来のSQL移行とユーザー認証対応に備えています。

#### 実装内容

- **12個のカケラデータ** - 6世界 × 2種類（Perfect/Normal）
- **3つの真実ルート** - 各ルート4個のカケラで解放
- **永続化システム** - `SaveManager`経由でLocalStorageに保存
- **能力復元** - セッション開始時に過去のカケラから能力を自動付与

#### アーキテクチャの特徴

```
ビジネスロジック層 → SaveManager (Repository) → LocalStorage/SQL DB
```
...
- `src/memory-fragments/fragments.ts` - カケラデータ定義
- `src/memory-fragments/truth-routes.ts` - 真実ルート定義
- `src/memory-fragments/award-logic.ts` - 授与・復元ロジック
- `src/save-system/save-manager.ts` - 永続化層（拡張済み）

### シーン進行システムの改善

**20ターン強制終了を削除**し、完全にシーンベースの進行管理に移行しました。

- **5つのシーンタイプ**: Introduction → Exploration → Conflict → Climax → Resolution
- **進行度ベース遷移**: 各シーンは目標ターン数と進行度で管理
- **柔軟な物語展開**: ターン制限に縛られない自然な物語進行

---

**現在のステータス**: Phase 3 進行中 - 記憶のカケラシステムの基礎実装完了。ギャラリーUI・タロットシステムを残す。
