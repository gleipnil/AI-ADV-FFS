...
export class GeminiClient {
     ...
     */
    async evaluateRoleplayBonus(
        playerIntent: string,
        judgmentRequest: JudgmentRequest
    ): Promise<RoleplayBonus> {
        const prompt = this.buildRoleplayEvaluationPrompt(playerIntent, judgmentRequest);

        try {
            const response = await this.model.generateContent(prompt);
            const text = await response.response.text();
            return this.parseRoleplayEvaluation(text);
        } catch (error) {
            console.error('GeminiClient: Failed to evaluate roleplay:', error);
            return { level: 0, modifier: 0, reasoning: '評価できませんでした' };
        }
    }

    private buildRoleplayEvaluationPrompt(
        playerIntent: string,
        request: JudgmentRequest
    ): string {
...
REASON: 慎重に構造を確認するという具体的なアプローチで、状況に適している。`;
    ...
    private parseRoleplayEvaluation(text: string): RoleplayBonus {
